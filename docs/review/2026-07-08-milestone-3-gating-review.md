...
- **Medium 5 ✅（2026-07-08 23:15）**：在线 `run()` 发送 game action 后记录 pending `actionId`，先显示“等待服务器确认”；仅在匹配的 `game.actionAccepted` 返回后清空选择并显示成功文案，`game.error` 则移除 pending 且保留当前选择。
