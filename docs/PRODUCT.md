...
MVP 当前已实现：加入成功后发放 resume token，客户端本地保存最近房间身份；重新连接同一房间时恢复原座位，并立即收到按该玩家视角生成的 snapshot。自动后台重连、超时暂停和无人房间清理后续补齐。
...
**移动优先（Mobile-First）**：本游戏的主要运行环境是手机。移动端不是缩小版的桌面端，而是默认目标；桌面端是移动端布局的增强放大版。所有界面、组件与交互必须先在手机竖屏上验证可用、可读、可点，再向上适配到平板与桌面。

移动优先的硬性约束：

- 先设计手机竖屏（约 375–430px 宽），再用 `min-width` 断点向上增强，避免 `max-width` 事后补丁。
- 触控优先：可点目标 ≥ 44×44px，间距足够，无悬停依赖的关键操作（hover 只作增强）。
- 单手可玩：核心操作（拿宝石、买卡、预留）集中在拇指可达的底部区域。
- 关键信息一屏可见：当前回合、公共宝石、三层市场、自己的资产、行动区不应被埋入深层菜单。
- 性能预算：控制首屏体积与图片尺寸，卡面等资源提供移动端更小的 runtime 版本（见 `docs/ui-overhaul-plan.md`）。
...
- 原创“兔拉崩吧”轻幻想主题（见 `art/bible/`）。
- 深色桌面 + 明亮宝石色点缀，但避免全屏单一紫蓝渐变。
- 卡牌信息密度高，桌游感强；卡面美术满铺、信息叠加显示（见 `docs/ui-overhaul-plan.md`）。
- 所有可行动作有明确 pressed / disabled / selected 状态；hover 仅作桌面增强。
...
- MVP 当前使用 `state.acceptWebSocket()`、WebSocket attachment 和 Durable Object storage 恢复连接身份与房间状态。
...
- MSW：前端接口 mock
- OpenClaw sandbox browser：仅作为关键节点隔离验收工具，不作为日常快速开发内循环。联机功能优先用 host browser + sandbox browser 组合模拟两个独立浏览器环境；避免多个子 agent 并行启动 sandbox browser，降低 Docker/WSL、CDP readiness、容器冷启动和超时风险。
...
4. 是否需要房主踢人/解散房间。
5. 原创视觉：**轻幻想卡牌 / 兔拉崩吧 IP**（已锁定，见 `art/bible/`）。
