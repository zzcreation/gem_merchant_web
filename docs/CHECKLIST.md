...
- [x] 平台入口模式：线上默认进入 Landing Page，提供游戏标题、加入房间、创建房间、选项、About 页面和版本号小标。
...
- [x] 移动端布局：手机端顶部压缩、玩家横向滑动、三层公共牌桌同屏可见、行动面板底部 sticky 抽屉化，并新增移动端 E2E 验收。
- [x] 游戏中 UI 精简：可购买市场卡强化高亮、预留卡显示折扣后的剩余成本、玩家卡片在 iPhone 15 Pro 宽度完整显示、房间级按钮不出现在玩家行动栏。
- [x] 错误提示与操作反馈：状态条区分 info / pending / success / error；在线行动 pending 防重复提交；本地规则错误不再打崩组件。
- [x] 服务器交互读取规则：凡是加入房间、创建房间、准备、开始、在线行动等与服务器通讯的操作，都显示 loading / pending indicator；加入房间等待服务端 lobby/snapshot 期间不渲染本地 mock 棋盘。
- [x] 预留牌体验修正：预留牌可在后续回合明确选择并购买；预留牌显示原始成本，不在牌面上显示折扣后剩余成本。
- [x] 当前回合提醒：轮到当前玩家时给出更明显的提示，尤其是轮到“你”时。
- [x] 结束结算面板：游戏结束后显示胜利者和简单统计数据。
- [ ] 动效、音效。
