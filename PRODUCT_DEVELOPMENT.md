...
- 已购买发展卡明细，至少可展开查看。
- 已预留卡牌明细。预留卡属于公开信息，所有玩家都能看到每名对手预留了哪些卡。
- 已获得贵族。
- 当前是否在线、是否轮到该玩家、是否处于弃宝石/选贵族等等待状态。

只有牌堆顶部盲抽前的未知卡牌是隐藏信息；一旦被玩家盲抽预留，该预留卡对所有玩家公开展示。
...
- Cloudflare Workers
- Durable Objects：房间状态和 WebSocket 协调
- D1：持久化已结束对局、房间索引、用户昵称记录
- KV：短期房间码映射和静态配置缓存
...
- 每个 action 必须具备幂等保护。
- 每个房间状态维护递增 `version`。
- 客户端断线重连时请求当前 snapshot。
- 所有随机洗牌使用 seed，可复现。
- 游戏结束后保存最终 snapshot 和行动日志。
...
- 预留市场卡后补牌。
- 预留盲抽在抽取前不向其他玩家展示具体卡牌；一旦进入玩家预留区，即作为公开预留卡向所有玩家展示。
- 预留上限 3。
- 黄金为空时仍可预留。
- 预留卡不可主动丢弃。
...
Browser
  ...
  -> D1: finished games / audit records
  -> KV: room code index / config cache
```
...
- 房间实时状态保存在 Durable Object 内存，并定期 snapshot。
- 结束对局才写 D1。
- 房间码映射写 KV，设置 TTL。
- 静态资源走 Pages。
...
1. 初始化项目脚手架。
2. 建立 `shared/game` 规则引擎目录。
3. 整理公开卡牌和贵族数据。
4. 写数据完整性校验测试。
5. 实现 setup 与基础 action reducer。
6. 设计第一版原创 UI wireframe。
7. 实现本地 mock 对战。
8. 接入 Durable Object 房间。
